...
public partial class MainWindow : Window
{
    ...
    private async void Start_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
    {
        ...
        var extensions = new[]
        {
            ...
            ".mp4", ".avi", ".mov", ".wmv", ".mkv", ".m4v", ".webm", ".flv", ".mpeg", ".mpg", ".3gp"
        };
        ...
        var dateCache = new ConcurrentDictionary<string, DateTime>();
        ...
        await Task.Run(() =>
        {
            Parallel.ForEach(files,
                ...
                filePath =>
                {
                    ...
                    try
                    {
                        (hash, size) = ComputeFileHashFast(filePath);
                    }
                    catch (Exception ex)
                    {
                        lock (_logBuilder)
                    ...
                    // cache date once
                    dateCache[filePath] = GetDateTakenSafe(filePath);
        ...
        await Task.Run(() =>
        {
            Parallel.ForEach(fileGroups.Values,
                new ParallelOptions { MaxDegreeOfParallelism = 4 },
                group =>
                {
                    var toKeepPath = group.First();
                    var date = dateCache.TryGetValue(toKeepPath, out var dt)
                        ? dt
                        : new FileInfo(toKeepPath).CreationTime;
                    ...
                    try
                    {
                        ...
                        Interlocked.Increment(ref organized);
                    }
                    catch (Exception ex)
                    {
                        lock (_logBuilder)
    ...
    private static (string hash, long size) ComputeFileHashFast(string filePath)
    {
    ...
    private DateTime GetDateTakenSafe(string filePath)
    {
        try
        {
